...
> **本节条款来自 Palantir 官方 `action-types/rules` 公开的规则模型。**
> 官方把它叫 **Rules**，共六种，并明确了几条硬约束。本规范取其**约束**，不改本规范既有的 `effects` 结构。

### 4.5.1 官方规则的六种（对照用）

| # | 官方规则 | 说明 |
|---|---|---|
| 1 | **创建对象** | "对象类型的主键是必填属性，必须填写" |
| 2 | **修改对象** | "**这不能引用作为当前操作一部分创建的对象**" |
| 3 | **删除对象** | 同上限制 |
| 4 | **创建链接** | "在通过对象引用参数传递的对象之间创建**多对多**链接" |
| 5 | **删除链接** | 同上 |
| 6 | **函数规则** | "**当此规则存在时，不能配置其他规则**" |

**官方同时明确**：**"对于外键链接，需要使用修改对象规则来显式修改外键属性"** ——
即：一对一 / 一对多链接**没有专门的创建/删除规则**。

| # | 条款 |
|---|---|
| **RL-1** | 实现 **MUST** 区分两种链接写入：**多对多链接**由"创建/删除链接"效果承担；**外键链接**由"修改对象"效果改外键属性承担。**MUST NOT** 用同一种效果覆盖两种情形 |
| **RL-2** | 若某效果引用了函数（`function` 类效果），该动作 **MUST NOT** 同时声明其他效果 —— **函数效果独占**（官方原文："当此规则存在时，不能配置其他规则"） |

> **`RL-2` 的理由（官方）**："仅函数代码就能够处理其他规则可以执行的所有操作" ——
> 混用会让两套效果对同一属性竞争，而**裁决规则不在声明里**（见 `EF-2`）。
...
| # | 条款 |
|---|---|
| **EF-3** | 实现 **MUST** 拒绝上述三种组合，**MUST NOT** 用"按顺序执行"的方式容忍。拒绝 **MUST** 在**定义校验**（构建期或提交前）完成 |

### 4.5.3 同一属性被多条效果修改：裁决必须声明（`EF-2`）★

**官方行为（逐字）**：

> **"如果一个规则的结果将属性更新为'A'，但同一操作类型中的另一个规则将同一对象的属性更新为'B'，则最终的编辑只会将属性更新为'B'。规则的顺序会影响最终的对象编辑。"**

**即：官方是"后者覆盖，且不报错"。**

| # | 条款 |
|---|---|
| **EF-2** | 当同一属性可能被**多条效果**修改时，实现 **MUST** 在**实现文档**中声明裁决规则，并 **MUST** 在**定义校验**时检测该情形。**MUST NOT** 静默覆盖而不声明 |

> **为什么本规范不直接采纳"后者覆盖"**：**静默覆盖**属于"不报错但结果不是你以为的"那一类问题 ——
> 定义者有两条效果写同一个属性，很可能本身就是错误。
> **本规范允许两种裁决**（后者覆盖 / 直接拒绝），但**要求显式声明并让校验器能查出**。
> **与官方一致的是"必须有确定结果"，比官方严的是"必须说出来"。**

### 4.5.4 效果的写入权限可判定性（`EF-1`）

**官方机制**：函数修改本体时，"**检测到的对象类型用于提供操作的来源，并可能被操作服务用于强制其权限**"（`functions/edits-overview`）。

| # | 条款 |
|---|---|
| **EF-1** | 每个效果 **MUST** 能判定它写入**哪些对象类型**；该判定结果 **MUST** 可用于**权限校验**。**MUST NOT** 存在"不知道会写到哪里"的效果 |
...
| **来自参数** | "与属性类型相同的现有参数" |
| **对象参数属性** | "现有对象引用参数的属性" |
| **静态值** | "仅存在于操作类型规则部分的静态值……该值不可更改" |
| **当前用户 / 时间** | "字符串和时间戳属性也可以采用操作当前用户或提交时间的上下文值" |

| # | 条款 |
|---|---|
| **EF-4** | 效果中每个被写入的值 **MUST** 来源于上述四种之一；**MUST NOT** 使用第五种未声明的来源 |

---

## 4.6 权限与业务条件必须分离（`PR-1` / `PR-2` / `EP-1`）★★★

> **官方原文（`action-types/submission-criteria`）**：
> **"这与管理用户是否可以编辑操作类型本身的权限无关。"**
>
> **即官方把两件事分成两套**：**提交标准**（业务条件）与**权限**（谁能做）。

**本规范的现状**：§2.3 的表达式上下文列了 `actor` 变量，意味着"不是 HR 组不许改"这类逻辑
**既可以写进 `preconditions`，也可以写成 `permissions`** —— **同一个权限逻辑有两个写入点。**

| # | 条款 |
|---|---|
| **PR-1** | 涉及"**谁可以做**"的判断 **MUST** 经 `permissions`（或引用的外部授权系统）；**MUST NOT** 写入 `preconditions` |
| **PR-2** | `permissions` **MUST** 能回答官方三问：**谁能查看**该动作 · **谁能编辑**该动作定义 · **谁能应用**该动作（含给定参数集） |
| **EP-1** | 实现 **MUST** 支持「**仅允许通过操作编辑**」这一对象类型编辑策略；**SHOULD** 将其作为新建对象类型的默认 |

**`actor` 变量的用途收紧**（对 §2.3 的修正）：

| 允许 | 禁止 |
|---|---|
| 业务性判断（如"提交人必须是该订单的负责人"） | 角色 / 组 / 权限判断（如"提交人必须在 HR 组"） |

> **为什么这条重要**：合并的后果不是"不规范"，而是**实现必然分裂** ——
> 同一个权限要求，一个实现写在 `preconditions`、另一个写在 `permissions`，
> 两边的**失败语义不同**（拒绝 vs 不可见），调用方无法统一处理。

**`EP-1` 的理由（官方原文，一条本框架此前未提出的理由）** ★★：

> **"对于仅允许通过操作进行编辑的 object 类型，提交操作的用户只需要对正在编辑的 object 具有`读取`权限。"**
> **"相反，当……可以通过操作、Foundry Forms、直接……编辑和 API 调用进行编辑时，提交操作的用户必须对所有正在编辑的 object 的数据输出数据集具有`编辑`权限。具有`编辑`权限的用户将能够查看数据输出数据集中的所有数据。"**
> **"因此，设置一个 object 类型通过操作、Foundry Forms、直接……编辑和 API 调用进行编辑是不鼓励的，因为仅为了 object 编辑而授予'编辑'权限可能会向用户暴露比完成 Ontology 编辑工作流程所需更多的数据。"**

**这条给出了「动作是唯一写入通道」的第三条理由**（前两条是治理与一致性）：

> **数据安全** —— 一旦开放操作之外的直编路径，写权限会**连带暴露整个数据集**。

**官方还有一条反直觉的推论，实现者必须知道**：

> **"对于仅允许通过操作进行编辑的 object 类型……这意味着用户可以创建他们无法查看的 object。"**

---

## 4.7 撤销语义（`RV-1`~`RV-3`）★

> **官方原文（`action-types/action-reverts`）**：**撤销不是回滚。**

| 官方限制 | 原文 |
|---|---|
| 谁能撤 | "**目前，操作只能由应用操作的用户撤销。**" |
| 时效 | "**只有当操作是对象的最新编辑时，才能撤销该操作。**"（针对**对象**，即使后续编辑改的是别的属性） |
| 开关 | "**如果在操作提交后操作撤销被关闭，即使之后再次开启，也无法撤销该操作。**" |
| 撤什么 | "**操作撤销仅撤销对象实例的编辑，但不会撤销副作用，如通知或 webhooks，也不会像应用操作那样调用它们。**" |

| # | 条款 |
|---|---|
| **RV-1** | 撤销 **MUST** 仅撤销**实例编辑**；**MUST NOT** 撤销副作用，也 **MUST NOT** 把撤销本身当作一次新的动作提交 |
| **RV-2** | 一次动作 **MUST** 仅在它仍是**该对象的最新编辑**时可被撤销；实现 **MUST** 能判定这一点 |
| **RV-3** | 撤销能力 **MUST** 在动作提交时确定；**MUST NOT** 支持事后开启对历史提交的撤销 |

> **`RV-1` 与框架的关系**：框架 §10 的审计要素里有"可回滚"一项。
> **该措辞 MUST 被理解为"可撤销"**，且**不覆盖副作用** —— 副作用需要的是**补偿**（第三件事，`RV-1` 不涉）。
>
> **`RV-3` 的理由**：事后开启撤销会破坏"当时的契约" ——
> 动作提交时若未声明可撤，其后来的撤销就没有依据。
...
| **AU-7** | 记录 **MUST** 含 `action_version` 与 `definition_hash` **两者**。`action_version` 是**定义版本**（人可读、随定义更新递增），`definition_hash` 是**内容指纹**（防"同版本号不同内容"）。**MUST NOT** 用其一替代另一 |
| **AU-8** | 多目标提交（`MI-1`~`MI-7`）**MUST** 产生**一条**记录，其中列出**全部**被写入实例；**MUST NOT** 按目标拆成多条 |

> **`AU-7` 的来源**：Palantir 官方操作日志把 `操作类型版本` 列为默认字段，并注明它是
> "**每次更新操作类型时自动递增的版本号**"——即版本与定义同步递增。本规范额外要求 `definition_hash`，
> 因为**版本号可以被人工复用，哈希不能**。
>
> **`AU-8` 的来源**：Palantir 官方操作日志"**与操作类型一一对应**"，且其例子中
> "一次关闭 10 个 `警报` 对象将产生**一个** `操作日志` 对象，并通过外键链接到所有 10 个"。
> 本规范采纳这一形态；**是否把该记录本身建模为可查询对象（官方做法）属实现选择**。
...
| **审计** | AU-1 成功同事务 · AU-2 拒绝也记 · AU-3 不可改 · AU-4 含定义哈希 · AU-5 含快照 · AU-6 SHOULD 用 CloudEvents · **AU-7 版本与哈希并存** · **AU-8 多目标一条记录** |
| **效果裁决** | **EF-1 效果 MUST 可判定写入权限** · **EF-2 同属性被多条规则改时裁决 MUST 声明** · **EF-3 非法组合 MUST 拒绝** · **EF-4 值来源 MUST 限于四种** |
