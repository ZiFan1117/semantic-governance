...
`Q-3` 要求把两者拆开。参考实现的做法是给审计记录增加一个 `change_record_id`：
...
| **v0.2.2** | 2026-09 | **新增 §4.3：拒绝 MUST 穿过适配层**（`X-1`~`X-3`）。依据 bazidiy 参考实现第二次回写：原子返回了 `rejection`，但工具 `output.schema`（`additionalProperties:false`）没声明它，**结构化拒绝在适配层被静默丢弃**，LLM 只看到"成功的空结果"。§8 增补第 10/11 条运行期可判定的断言 |
...
| **v0.2.4** | 2026-09 | **§3.4 增补 `Q-3`**：`Q-2` 让 `record_id` 在两个位置指代不同东西（返回结果里指**变更**、审计记录里指**调用**），调用方无法区分。`Q-3` 要求拆成两个字段（参考实现用 `change_record_id`），使"审计条数=调用数、变更 id 去重=变更数"成为可机读的事实，而不是需要猜的约定 |
